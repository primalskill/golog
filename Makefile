...
test: format lint
	clear && printf '\e[3J'	
	go clean -testcache
	go test -count=1 -cover -v -failfast -run ${TESTREGEX} ${TESTPATH}

.PHONY: test-cover
test-cover: format lint
	clear && printf '\e[3J'	
	go clean -testcache 
	go test -coverprofile /tmp/cover.out -v -run ${TESTREGEX} ${TESTPATH}
